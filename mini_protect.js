(async () => {
  ...
  m_banLimit = 20
  bot.on('ready', () => {
    const RESETBAN = setInterval(function () {
      m_ban = 0
      ...
      if (m_ban < m_banLimit) {
        ...
        axios.get(`https://discord.com/api/v9/guilds/${config.guild}/audit-logs?ilimit=1&action_type=22`, {
          ...
          if (response.data && response.data.audit_log_entries[0].user_id) {
            if (response.data.audit_log_entries[0].user_id === arrBots[intB].user.id) {
            } else {
              // -- Ban
              axios({
        ...
        // -- ping
        const terminal = client.channels.cache.find(c => c.name === '_terminal')
        if (!terminal) return
